...
for i in $(ls $PWD/*.tpch.*.sql); do
	for x in $(seq 1 $SINGLE_USER_ITERATIONS); do
		...
		if [ "$EXPLAIN_ANALYZE" == "false" ]; then
			echo "psql -d $DBNAME -v ON_ERROR_STOP=1 -A -q -t -P pager=off -v EXPLAIN_ANALYZE=\"\" -f $i | wc -l"
			tuples=$(psql -d $DBNAME -v ON_ERROR_STOP=1 -A -q -t -P pager=off -v EXPLAIN_ANALYZE="" -f $i | wc -l; exit ${PIPESTATUS[0]})
		else
			myfilename=$(basename $i)
			mylogfile=$PWD/../log/$myfilename.single.explain_analyze.log
			echo "psql -d $DBNAME -v ON_ERROR_STOP=1 -A -q -t -P pager=off -v EXPLAIN_ANALYZE=\"EXPLAIN ANALYZE\" -f $i > $mylogfile"
			psql -d $DBNAME -v ON_ERROR_STOP=1 -A -q -t -P pager=off -v EXPLAIN_ANALYZE="EXPLAIN ANALYZE" -f $i > $mylogfile
			tuples="0"
		fi
		log $tuples
	done
