...
- Added `REPO_BRANCH` parameter to select the git branch used by `tpch.sh` (default: `master`)
...
- REPO_BRANCH="master" — git-ветка репозитория для `tpch.sh` (`repo_init`). По умолчанию `master`; для кастомной укажите имя, например `ADPG-5212`
- ADMIN_USER="gpadmin"
